module.exports = {
    ...
    post: async (req, res) => {
        ...
        const db = req.app.get("db");
        const comments = await db.comments.post_comment([user_id, route_id, trail_conditions, comment]);
        return res.send(comments);
    },
    editComment: async (req, res) => {
        const {comment_id, trail_conditions, comment} = req.body;
        const db = req.app.get("db");
        const comments = await db.comments.edit_comment([comment_id, trail_conditions, comment]);
        return res.send(comments);
    },
